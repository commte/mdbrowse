...
```sh
mdb -w file.md   # 変換して、保存のたびに変換し直す
mdb file.md      # 1回だけ変換する
...
ターミナルで1回起動して、そのまま置いておく

```sh
mdb -w -o file.md
```

プレビュー用のタブを開いたうえで、保存のたびに変換し直す。エディタ側の設定は要らず、どのエディタで保存しても追従する。止めるときは `Ctrl-C`。スタイルシートを直したときも作り直すので、見た目を調整している間も使える

`-w` を付けなければ1回変換して終わる。常駐するものは残らず、ファイルの監視もしない。タブが変わるのはコマンドが走ったときだけになる。エディタのショートカットから呼ぶときはこちら
...
`mdb -w` ならエディタ側の設定は要らない。ターミナルを開いておくよりキーを押すほうがよければ、こちらを設定する
...
```
mdb -w <file>（またはエディタのショートカット） → pandoc → /tmp/mdbrowse.html
...
監視は同じ変換を繰り返しているだけで、`mdb -w` はファイルの更新時刻とサイズを1秒ごとに見て、変わったら変換し直す。ふつうの前面プロセスであってサーバーではない。ポートも無く、止めるのは `Ctrl-C` だけ
